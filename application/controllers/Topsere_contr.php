...
class Topsere_contr extends CI_Controller {

    public function __construct()
    {
        parent::__construct();
        $this->load->model('Topsere_mdl','topsere_contr');
    }
    ...
    public function index()
    {
        ...
        if ($session == TRUE) {
            ...
            $data['userss'] = $this->topsere_contr->gettopi();
            $data['xnilai'] = $this->topsere_contr->ratarata();
            $user = $this->topsere_contr->gettopi();

            foreach ($user as $key => $value) {
                $xnilai = $this->topsere_contr->nilaibaru($value['idpeserta'])->row();
                $data['userss'][$key]['xnilai'] = $xnilai->xnilai;
            }
            //var_export($data['userss']);
            $data['prod'] = $this->topsere_contr->getprod();
            $data['iot'] = $this->topsere_contr->getiot();
            $data['level'] = $this->topsere_contr->getlevel();
            $this->load->view('biv/Topsere_view',$data);
        } else {
            redirect('depan');
        }
