...
class Screne_mdl extends CI_Model {
...
public function getalluser($idscreen) {
    $this->db->select('anggota.*,screentahap.*,users.*');

    $this->db->from('anggota');
    $this->db->join('screentahap', 'screentahap.idscreener = anggota.idscreen');
    $this->db->join('users', 'screentahap.idpeserta = users.id');
...
public function ambilin($idrelation) {
       // $this->db->select('anggota.*,screenrelation.*');
        ...
        $this->db->where('screentahap.idrelation', $idrelation);
        $query = $this->db->get();
        return $query->result_array();
    }
