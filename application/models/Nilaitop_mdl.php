...
class Nilaitop_mdl extends CI_Model {
...
public function getalluser($idscreen = NULL, $level = NULL) {
    //$this->db->select('users.*,screentahap.*');
    if($level != '5') {
        $this->db->select('users.*,screentahap.*');
    } else {
        $this->db->select('users.*,screentahap.*,anggota.nama');
    }
    $this->db->from($this->table);
    $this->db->join('screentahap', 'screentahap.idpeserta = users.id');
    if($level == '5') {
        $this->db->join('anggota', 'screentahap.idscreener = anggota.idscreen');
    }
    if($idscreen) {
        $this->db->where('idscreener', $level);
    }
